  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Hozzáadás"  onPress={() => {handleAddPerson()}} />
        ...
        renderItem={({item}) => (
          <View style={{borderBottomWidth: 1, borderBottomColor: "gray", padding: 10, marginTop:10, width:300}}>
            <Text style={{fontSize:18, fontWeight:"bold"}}>Név: {item.name}</Text>
            <Text style={{fontSize:16}}>Email: {item.email}</Text>
            <Text style={{fontSize:16}}>Életkor: {item.age}</Text>
          </View>
        )
        />
